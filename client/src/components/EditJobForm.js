// components/EditJobForm.js
import React, { useEffect, useState, useContext } from 'react';
import { useNavigate, useParams, Link } from 'react-router-dom';
import { Survey } from 'survey-react-ui';
import { Model } from 'survey-core';
import { editJobFormJson } from '../surveyForms/editJobForm';
...
const EditJobForm = () => {
  ...
  const [surveyModel, setSurveyModel] = useState(new Model(editJobFormJson));
  ...
  useEffect(() => {
    ...
    const fetchJobEntry = async () => {
      try {
        const response = await fetch(getApiUrl(`/api/job/${id}`));
        const data = await response.json();
        console.log('Fetched Job Entry:', data);
    ...
    survey.onChoicesLazyLoad.add((sender, options) => {
      ...
      if (questionName === 'company') {
        ...
        endpoint = getApiUrl(`/api/designations?filter=${searchText}&skip=${skip}&take=${take}`);
      }
  ...
  return (
    <div className="signup-container">
      ...
      <div className="signup-main-content">
        <h2>EDIT YOUR JOB DETAILS</h2>
      </div>
      {loading ? (
        <div className="loader-container">
          <div className="loader"></div>
          <div className="loader-message">Saving your job details...</div>
        </div>
      ) : (
