// components/JobsForm.js
import React, { useEffect, useState, useContext } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { Survey } from 'survey-react-ui';
import { Model } from 'survey-core';
import { jobsFormJson } from '../surveyForms/jobsForm';
...
const JobsForm = () => {
  ...
  const [surveyModel, setSurveyModel] = useState(new Model(jobsFormJson));
    ...
    survey.onComplete.add(async(sender) => {
      ...
      try {
        const responses = await Promise.all(entries.map(async entry => {
          ...
          const response = await fetch(getApiUrl('/api/addJob'), {
            method: 'POST',
            headers: {
